...
void respuestas(char buffer[256], int *ultimaMuestra, char missatge[255],
		int fd, char buf[255], float arrLecturas[3600]);
...
int adquirirMedia(int *ultimaMuestra, int fd, char buf[255],
		  float arrLecturas[3600]);
...
void *trhead_solicitudMedias()
{
	...
	printTime();
	printf(COLOR_GREEN "Esperando comunicacion Serie\n" COLOR_RESET);
	while (1) {
		if (tomadeMuestras == 1) {
			if (led == '0') {
				led = '1';
...
int main(int argc, char *argv[])
{
	...
	while (1) {
		...
		printTime();
		printf(COLOR_RESET "Servidor esperant connexions\n");

		/*Esperar conexió. sFd: socket pare, newFd: socket fill*/
		newFd = accept(sFd, (struct sockaddr *)&clientAddr, &sockAddrSize);
		printTime();
		printf(COLOR_RESET "Connexión acceptada del client: adreça %s, port %d\n",
		       inet_ntoa(clientAddr.sin_addr), ntohs(clientAddr.sin_port));
		/*Rebre*/
		result = read(newFd, buffer, 256);
		printTime(); printf(COLOR_RESET "Missatge rebut del client(bytes %d): %s\n", result, buffer);
		if (result > 0) {
			// Si hay entrada de datos por la comunicación TCP
			// ejecutamos sus respuestas
			respuestas(buffer, &ultimaMuestra, missatge, fd, buf, arrLecturas);
			/*Enviar*/
			strcpy(buffer, missatge);               // Copiar missatge a buffer
			result = write(newFd, buffer,
				       strlen(buffer) + 1);     //+1 per enviar el 0 final de cadena
...
int adquirirMedia(int *ultimaMuestra, int fd, char buf[255], float arrLecturas[3600])
{
	...
	int mediaRecibida = (buf[3] - 48) * 1000 + (buf[4] - 48) * 100 +
			    (buf[5] - 48) * 10 + (buf[6] - 48);
	// Calculamos la temperatura multiplicando el integuer por la sensibilidad del
	// sensor
	float temperatura = mediaRecibida * 0.048;

	// Si la recepcion Serie ha sido correcta, guardamos la muestra. En caso de
	// error, no se guardara el dato.
	if (errorRecepcion == 0 && (mediaRecibida > 0 && mediaRecibida < 1024)) {
		...
		*ultimaMuestra = *ultimaMuestra + 1;
		if (*ultimaMuestra > 3599) {
			*ultimaMuestra = 0;
		} else {
			printTime();
			printf("NO SE GUARDARA EL DATO RECIBIDO");
			temperatura = -1;
		}
		return temperatura;
	}
	...
	if (buffer[0] == 'A' && (buffer[2] == 'Z' || buffer[6] == 'Z')) {
		...
		case ('U'):         // Solicitar la media("AUZ")
		{
			for (int i = 0; i < *ultimaMuestra; i++) {
				...
				printTime();
				sprintf(missatge, "AU%i%02.2fZ", codigoRetorno, media);
				break;
			}
		}
