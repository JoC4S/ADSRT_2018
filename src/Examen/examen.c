...
char* timestamp(int print)
{
	...
	/******MODIFICACION EXAMEN******************
	/** SE AÑADEN OPCIONES A LA FUNCION PARA QUE DEVUELVA SOLO LA DATA O LA HORA SEGÚN LE LLEGUE UN 2 O UN 3 RESPECTIVAMENTE*/
	if (print == 1)
...
int dbfunc(int opcion, char *nombredb)
{
	...
	case 1: /** Tabla de DATOS para datos capturados e información del estado del ventilador*/
		...
		sql = "CREATE TABLE DATOS (HORA DATETIME  NOT NULL,TEMPERATURA FLOAT NOT NULL, VENTILADOR INT NOT NULL);" \
		      ...
		      "CREATE TABLE Registre_Temp_Exam (Data DATETIME  NOT NULL, Hora NOT NULL, Temp FLOAT NOT NULL);";
		printf(" |--->Crear tabla 'DATOS'.\n");
		printf(" |--->Crear tabla 'ALARMAS'.\n");
		printf(" |--->Crear tabla 'Registre_Temp_Exam'.\n");
		break;
	case 2:
		...
		/*****MODIFICACION EXAMEN****************************
		/** Prepara los datos para rellenar la tabla REgistre_Temp_Exam*/
		char horamuestreo_data[20];
		char horamuestreo_hora[20];
		strcpy(horamuestreo_data,timestamp(2));
		strcpy(horamuestreo_hora,timestamp(3));
		sqlite3_prepare_v2(db, "insert into Registre_Temp_Exam (Data, Hora, Temp) values (?1, ?2, ?3);", -1, &stmt, NULL);
		sqlite3_bind_text(stmt, 1, horamuestreo_data, -1, SQLITE_STATIC);
		sqlite3_bind_text(stmt, 2, horamuestreo_hora, -1, SQLITE_STATIC);
		sqlite3_bind_double(stmt, 3, mustratemperatura);
		/*****MODIFICACION EXAMEN****************************
		...
		if (rc != SQLITE_DONE) {
			...
			/*****MODIFICACION EXAMEN****************************
			/** Imprime por pantalla los datos metidos en la tabla Registre_Temp_Exam*/
			printf("\nIntroducidos en tabla 'Registre_Temp_Exam:'\n" );
			printf("----> Data: %s'\n", horamuestreo_data );
			printf("----> Hora: %s'\n", horamuestreo_hora);
			printf("----> Temp: %2.2f ºC\n",mustratemperatura);
			/** Enciende o apaga el led de arduino.*/
			char orden[7];
			/** Activa o desactiva el el led 7 tras guardar los datos en la nueva tabla*/
			strcpy(orden, "AS");
			strcat(orden, "07");
			/*Rnunciado Examen*/
		//	if (led_onoff == 0){
