...
int timeout = 100;                          //tiempo de timeout para el Serieal.readstring()
String codiRetorn;
...
void loop()
{
        if (Serial.available() > 0)                                           //Si hi ha alguna cosa escrita = 1
        {
                ...
                if (msgIn[0] == 'A')
                {
                        ...
                        case 'T':                                                     //ATZ comando para debug comunicacion
                                                                                      //Modifica  Serial.setTimeout(100) para provocar Un timeout el programa de Reaspberry
                                codiRetorn = checkMsgC(msgIn);
                                msgOut = "AT" + codiRetorn +"Z";
                                if (timeout < 200)
                                        timeout = 3500;
                                else
                                        timeout = 100;
